...
int main() {
  ...
  server_pollfd.events =
      POLLIN; // Bitmap of events we are interested in (ready to read/accept)
  server_pollfd.revents = 0; // Bitmap filled by OS

  fds.push_back(server_pollfd);

  while (true) {
    // poll() blocks here until at least one file descriptor has an event, or it
    // times out. The first parameter is the underlying array of our vector,
    // second is the size, third is timeout (-1 means wait indefinitely).
    ...
    for (size_t i = 0; i < fds.size(); ++i) {
      ...
      if (fds[i].revents & POLLIN) {
        ...
        // Case A: The event happened on the SERVER socket (A new client is
        // connecting)
        ...
        // Case B: The event happened on a CLIENT socket (An existing client
        // sent us data)
        else {
          char buffer[1024] = {0};
          ...
          std::cout << "Method: " << parsed_req.method
                    << " URI: " << parsed_req.uri << std::endl;
          ...
          if (bytes_read <= 0) {
            ...
            // We received data! For now, just print it and send back our
            // greeting.
            ...
            // HTTP/1.1 usually keeps connections alive, but to keep this
            // simple, we'll close it after replying.
            close(fds[i].fd);
            fds.erase(fds.begin() + i);
            --i;
          }
