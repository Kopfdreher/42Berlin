...
- `path`: `cgi_path` (e.g. `/usr/bin/python3`)
- `argv`: the strArr, `argv[0] = path`, `argv[1] = SCRIPT_FILENAME`, `argv[2] = NULL`
...
**Note on `PATH_INFO` vs `SCRIPT_NAME**`: Be careful with the logic. If the URL is `/cgi-bin/test.php/extra/path`, the `SCRIPT_NAME` is `/cgi-bin/test.php` and the `PATH_INFO` is `/extra/path`. Many students forget to split these correctly!